        ...
        if self.reward_classifier_func is not None:
            ...
            if "image_primary" in obs:
                 ...
                 if img.ndim == 3: # (H, W, C)
                      ...
                      pass # Assuming batch dim is there, just check time dim?
                      # For safety, let's assume if it's 3D coming from env step, it needs full expansion.
                 
                 temp_obs["image_0"] = img
            else:
                 # Fallback if image_primary is missing (should not happen in correct config)
                 temp_obs = obs.copy()
