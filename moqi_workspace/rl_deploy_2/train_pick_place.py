...
flags.DEFINE_string("demo_path", "/home/peter/Desktop/zy/moqi_workspace/rl_deploy_2/rl_success_demos.pkl", "Path to the demo data.")
flags.DEFINE_integer("checkpoint_period", 1000, "Period to save checkpoints.")
flags.DEFINE_string("checkpoint_path", "/home/peter/Desktop/zy/moqi_workspace/rl_deploy_2/checkpoints", "Path to save checkpoints.")
...
def main(_):
    ...
    # Define Config with Cameras to trigger Observation Space creation
    # Keys must match what _update_currpos produces ("image_primary", etc.)
    class TrainConfig(DefaultOpenArmConfig):
        REALSENSE_CAMERAS = {
            "image_primary": "real_camera_serial_or_empty", # <--- Use Real Camera
    ...
    env = OpenArmEnv(
        ...
        arm="right",
        hz=5, # <--- MODIFY CONTROL FREQUENCY (HZ) HERE
        config=TrainConfig()
    )
    
    # Wrappers (Must match record_demo.py)
