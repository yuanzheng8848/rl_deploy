...
class OpenArmEnv(gym.Env):
    def __init__(
        ...
        self.target_pos = None
        self.target_offset = np.array([0.1, 0.0, 0.0]) # Forward 10cm task
    ...
    def step(self, action: np.ndarray) -> tuple:
        ...
        for i in range(2):
            if i in arm_indices:
                ...
                # Inactive arm: Keep current gripper pos?
                # Or just default to Open (-1.04) or Closed (0.0)?
                # Let's use the last known gripper pos from self.curr_gripper_pos
                # But self.curr_gripper_pos is [0, 0] initially.
                # Let's just send the current value we have in memory to avoid movement.
                # self.curr_gripper_pos is updated in _update_currpos from server.
        ...
        # 计算奖励与结束标志
        ob = self._get_obs()
        reward = self.compute_reward(ob, False) 
        
        # Success Condition: Distance < 5cm
        if self.arm == "right":
            current_pos = ob["state"]["tcp_pose"][:3]
        elif self.arm == "both":
            current_pos = ob["state"]["tcp_pose"][1, :3]
        else:
            current_pos = ob["state"]["tcp_pose"][:3]
        ...
        done = self.curr_path_length >= self.max_episode_length or success
        
        return ob, reward, done, False, {"success": success, "dist": dist}

    def compute_reward(self, obs: Dict, gripper_effective: bool) -> float:
        """
        计算奖励 (平滑奖励)
        目标：右臂到达初始位置前方 10cm 处
        Reward = 1.0 - tanh(10 * dist)
        """
        # 获取当前右臂 TCP 位置 (XYZ)
        if self.arm == "right":
            current_pos = obs["state"]["tcp_pose"][:3]
        elif self.arm == "both":
            current_pos = obs["state"]["tcp_pose"][1, :3]
        else:
            current_pos = obs["state"]["tcp_pose"][:3] # Left arm default
        ...
        if self.target_pos is not None:
            ...
            # 打印调试信息
            if dist < 0.05:
                print(f"\n[Reward] SUCCESS! Dist: {dist:.4f}. Reward: {reward:.4f}\n")
            else:
                # print(f"\r[Reward] Dist: {dist:.4f} Reward: {reward:.4f}", end="")
                pass
    ...
    def reset(self, **kwargs):
        ...
        # Set Target based on initial position
        if self.arm == "right":
            initial_pos = self.currpos[1, :3]
        elif self.arm == "both":
            initial_pos = self.currpos[1, :3] # Use right arm as reference
        else:
            initial_pos = self.currpos[:3]
            
        self.target_pos = initial_pos + self.target_offset
        print(f"[Env] Reset Done. Target Pos: {self.target_pos}")
